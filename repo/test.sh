...
# triple nested single-line
_triple=""
_v=1
switch "x"
  default: if $_v == 1: _triple="deep"
end
assert_eq "triple single-line" "$_triple" "deep"
...
echo ""
echo "=== Assignment Syntax ==="

str_after "hello/world" "/"
_old_way="$R"
_new_way = str_after "hello/world" "/"
assert_eq "basic assignment" "$_new_way" "$_old_way"

_chain1 = str_after "a/b/c" "/"
_chain2 = str_after "$_chain1" "/"
assert_eq "chained assignment" "$_chain2" "c"

str_before "key=value" "="
_old_before="$R"
_new_before = str_before "key=value" "="
assert_eq "multi arg assignment" "$_new_before" "key"

_inside_if=""
if 1 == 1
  _inside_if = str_after "path/to/file" "/"
end
assert_eq "assignment in if" "$_inside_if" "to/file"

_while_result=""
_while_i=0
while $_while_i < 1
  _while_result = str_after "x/y" "/"
  _while_i++
done
assert_eq "assignment in while" "$_while_result" "y"

_for_result=""
for _item in single
  _for_result = str_after "a/b" "/"
done
assert_eq "assignment in for" "$_for_result" "b"

_test_assign_func() {
  _func_result = str_after "$1" "/"
  R="$_func_result"
}
_test_assign_func "foo/bar"
assert_eq "assignment in function" "$R" "bar"

_quoted_eq = str_after "a = b" " "
assert_eq "quoted string with =" "$_quoted_eq" "= b"

_quoted_deq = str_before "x == y" " "
assert_eq "quoted string with ==" "$_quoted_deq" "x"

_my_var = str_after "1/2" "/"
assert_eq "underscore var" "$_my_var" "2"

_var123 = str_after "a/b" "/"
assert_eq "var with numbers" "$_var123" "b"

_cmp_test="no"
if "a = b" == "a = b"
  _cmp_test="yes"
end
assert_eq "comparison not assignment" "$_cmp_test" "yes"

_nospace="original"
_nospace=changed
assert_eq "no space not assignment" "$_nospace" "changed"

map_get nonexistent_map nonexistent_key
_empty_old="$R"
_empty_new = map_get nonexistent_map nonexistent_key
assert_eq "empty result assignment" "$_empty_new" "$_empty_old"

_input="hello/world"
_delim="/"
_var_args = str_after "$_input" "$_delim"
assert_eq "variable args" "$_var_args" "world"

map_set _test_map key "value with spaces"
_special_result = map_get _test_map key
assert_eq "special chars in result" "$_special_result" "value with spaces"

array_clear _qa
array_add _qa "first"
array_add _qa "second"
_qa_result = array_get _qa 0
assert_eq "array get assignment" "$_qa_result" "first"

_ma1 = str_after "a/b" "/"
_ma2 = str_after "c/d" "/"
assert_eq "sequential assign 1" "$_ma1" "b"
assert_eq "sequential assign 2" "$_ma2" "d"

_deep=""
if 1 == 1
  if 2 == 2
    if 3 == 3
      _deep = str_after "x/y/z" "/"
    end
  end
end
assert_eq "deeply nested assignment" "$_deep" "y/z"

_switch_assign=""
switch "test"
case test
  _switch_assign = str_after "m/n" "/"
end
assert_eq "assignment in switch" "$_switch_assign" "n"

_trim_assign = str_trim "  hello  "
assert_eq "str_trim assignment" "$_trim_assign" "hello"

_indent_assign = str_indent "  hello"
assert_eq "str_indent assignment" "$_indent_assign" "  "

_before_nope = str_before "noslash" "/"
assert_eq "str_before no match" "$_before_nope" "noslash"

_after_nope = str_after "noslash" "/"
assert_eq "str_after no match" "$_after_nope" "noslash"

_cond_assign = str_after "yes/no" "/"
if "$_cond_assign" == "no"
  pass "assignment then conditional"
else
  fail "assignment then conditional"
end

array_clear _len_test
array_add _len_test "a"
array_add _len_test "b"
_len_result = array_len _len_test
assert_eq "array_len assignment" "$_len_result" "2"

_try_assign=""
try
  _try_assign = str_after "p/q" "/"
catch
  _try_assign="error"
end
assert_eq "assignment in try" "$_try_assign" "q"

eval '1var = str_after "a/b" "/"' 2>/dev/null
ret=$?
if $ret != 0
  pass "rejects numeric-start var"
else
  fail "accepted numeric-start var"
end

str_after "old/style" "/"
_old_r="$R"
assert_eq "old R style works" "$_old_r" "style"
