...
_job_is_name() {
  ...
  case "$1" in
    sh|bash|dash|zsh|ksh|awk|sed|grep|find|cat|echo|printf|ls|cp|mv|rm|mkdir|touch|chmod|chown|head|tail|sort|uniq|wc|tr|cut|date|sleep|kill|test|true|false|cd|pwd|env|export|read|eval|exec|exit|wait|trap|set|unset|curl|wget|tar|gzip|zip|unzip|make|git|ssh|scp|rsync|docker|npm|node|python|ruby|perl|php) return 1 ;;
  esac
  return 0
}

job() {
  _shsh_job_n=$((_shsh_job_n + 1))
  _job_id="_$_shsh_job_n"

  case "$1" in
    -*|*/*|*=*) ;;
    *)
      if [ $# -gt 1 ] && _job_is_name "$1" "$2"; then
        _job_id="$1"; shift
      fi
      ;;
  esac
  ...
  eval "__shsh_job_${_job_id}_pid=$_job_pid"
  _shsh_jobs="$_shsh_jobs $_job_id"
}
...
job_capture() {
  ...
  eval "__shsh_job_${_job_id}_pid=$_job_pid"
  eval "__shsh_job_${_job_id}_log=\"$_job_log\""
  _shsh_jobs="$_shsh_jobs $_job_id"
}
...
job_wait() {
  if [ -n "$1" ]; then
    _shsh_check_name "$1" || return 1
    eval "_jw_pid=\"\${__shsh_job_${1}_pid}\""
    eval "_jw_log=\"\${__shsh_job_${1}_log}\""
    [ -z "$_jw_pid" ] && return 1

    wait "$_jw_pid" 2>/dev/null
    R=$?

    if [ -n "$_jw_log" ] && [ -f "$_jw_log" ]; then
      cat "$_jw_log"
      rm -f "$_jw_log"
    fi

    unset "__shsh_job_${1}_pid" "__shsh_job_${1}_log"
    _shsh_jobs="${_shsh_jobs% $1}${_shsh_jobs#* $1}"
  else
    R=0
    for _jw_id in $_shsh_jobs; do
      eval "_jw_pid=\"\${__shsh_job_${_jw_id}_pid}\""
      eval "_jw_log=\"\${__shsh_job_${_jw_id}_log}\""
      [ -z "$_jw_pid" ] && continue

      wait "$_jw_pid" 2>/dev/null
      _jw_code=$?
      [ "$_jw_code" -ne 0 ] && R=$_jw_code
      ...
      unset "__shsh_job_${_jw_id}_pid" "__shsh_job_${_jw_id}_log"
    done
    _shsh_jobs=""
  fi
}
...
job_status() {
  _shsh_check_name "$1" || return 1
  eval "_js_pid=\"\${__shsh_job_${1}_pid}\""
  [ -z "$_js_pid" ] && return 1
  kill -0 "$_js_pid" 2>/dev/null
}

job_running() {
  for _jr_id in $_shsh_jobs; do
    eval "_jr_pid=\"\${__shsh_job_${_jr_id}_pid}\""
    [ -n "$_jr_pid" ] && kill -0 "$_jr_pid" 2>/dev/null && return 0
  done
  return 1
}
...
job_count() {
  R=0
  for _jc_id in $_shsh_jobs; do
    eval "_jc_pid=\"\${__shsh_job_${_jc_id}_pid}\""
    [ -n "$_jc_pid" ] && kill -0 "$_jc_pid" 2>/dev/null && R=$((R + 1))
  done
}

job_each() {
  _je_arr="$1"; _je_fn="$2"
  _shsh_check_name "$_je_arr" || return 1
  eval "_je_len=\${__shsh_${_je_arr}_n:-0}"
  _je_i=0
  while [ "$_je_i" -lt "$_je_len" ]; do
    eval "_je_val=\"\${__shsh_${_je_arr}_$_je_i}\""
    job "$_je_fn" "$_je_val"
    _je_i=$((_je_i + 1))
  done
}
  ...
  *" | "*|*" && "*|*" || "*)
  ...
  "job: "*)
    ...
    *)
    ...
    ;;
  "job "*": "*)
    R=${stripped#*"job "}; [ "$R" != "$stripped" ]; _job_rest="$R"
    R=${_job_rest%%": "*}; [ "$R" != "$_job_rest" ]; _job_name="$R"
    R=${_job_rest#*": "}; [ "$R" != "$_job_rest" ]; _job_cmd="$R"
    case $_job_cmd in
    *"&&"*|*"||"*|*";"*|*"|"*)
      printf '%s\n' "${indent}job $_job_name sh -c '$_job_cmd'"
      ;;
    *)
      printf '%s\n' "${indent}job $_job_name $_job_cmd"
    ;;esac
    ;;
  "job_capture: "*)
    R=${stripped#*"job_capture: "}; [ "$R" != "$stripped" ]
    case $R in
    *"&&"*|*"||"*|*";"*|*"|"*)
      printf '%s\n' "${indent}job_capture sh -c '$R'"
      ;;
    *)
      printf '%s\n' "${indent}job_capture $R"
    ;;esac
    ;;
  "job_capture "*": "*)
    R=${stripped#*"job_capture "}; [ "$R" != "$stripped" ]; _job_rest="$R"
    R=${_job_rest%%": "*}; [ "$R" != "$_job_rest" ]; _job_name="$R"
    R=${_job_rest#*": "}; [ "$R" != "$_job_rest" ]; _job_cmd="$R"
    case $_job_cmd in
    *"&&"*|*"||"*|*";"*|*"|"*)
      printf '%s\n' "${indent}job_capture $_job_name sh -c '$_job_cmd'"
      ;;
    *)
      printf '%s\n' "${indent}job_capture $_job_name $_job_cmd"
    ;;esac
    ;;
  "try")
    try_depth_inc
